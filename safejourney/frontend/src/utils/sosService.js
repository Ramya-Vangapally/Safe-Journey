...
export const respondToSOS = async (alertId, helperId, helperLocation) => {
  try {
    const response = await fetch(`${BACKEND_URL}/api/sos/respond`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        alert_id: alertId,
...
export const resolveSOS = async (alertId, helperId, helperLocation, creditPoints = 100) => {
  try {
    const response = await fetch(`${BACKEND_URL}/api/sos/resolve`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        alert_id: alertId,
...
export const getActiveSOSAlerts = async () => {
  try {
    const response = await fetch(`${BACKEND_URL}/api/sos/active`)
