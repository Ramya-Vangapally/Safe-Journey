...
export const getSegmentReports = async (segmentId) => {
  try {
    const response = await fetch(`${BACKEND_URL}/api/segments/${segmentId}/reports`)
...
export const getSegmentByLocation = async (lat, lng) => {
  try {
    const response = await fetch(`${BACKEND_URL}/api/segments/by-location?lat=${lat}&lon=${lng}`)
